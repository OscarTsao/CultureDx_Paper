...
def main() -> None:
    ...
    if text.count(start_marker) != 1:
        raise RuntimeError("Expected exactly one Chapter 3 start marker")
    if text.count(end_marker) != 1:
        raise RuntimeError("Expected exactly one Chapter 4 start marker")
    ...
    chapter = updated[updated.index("\\chapter{Study Architectures and Diagnostic Workflow}"):updated.index(end_marker)]
    forbidden = [
        "Qwen3-32B",
        "4-bit AWQ",
        "vLLM",
    ...
    found = [token for token in forbidden if token in chapter]
    if found:
        raise RuntimeError(f"Implementation details remain in Chapter 3: {found}")

    target.write_text(updated, encoding="utf-8")
    print(f"Rewrote Chapter 3 in {target}")
