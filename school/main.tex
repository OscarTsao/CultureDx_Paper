...
The main results below use the matched method-comparison trace in
\texttt{results/mdd\_variant\_matrix/da/predictions.jsonl}. For each case, the benchmark gold diagnoses, committed primary diagnosis, Diagnostician ranking, and criterion-compatible set are taken from the same record. All labels are projected to the paper parent space. A case is counted as correct when the committed primary matches any projected benchmark gold parent, following the metric contract in Chapter~\ref{ch:data}.
...
\hline
\textbf{Population} & \textbf{Cases} & \textbf{Top-1} & \textbf{Genuine Top-3} & \textbf{Gold in compatible set} & \textbf{Median compatible-set size}\\
\hline
