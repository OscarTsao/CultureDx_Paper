...
Within-corpus Top-1 is higher than cross-corpus Top-1 in both transfer directions. Training on LingxiDiag and evaluating on MDD-5k reduces Top-1 from 0.602 to 0.059. Training on MDD-5k and evaluating on LingxiDiag reduces it from 0.632 to 0.476. The asymmetric degradation indicates that the classifier relies substantially on source-specific lexical regularities, dialogue-generation patterns, and label distributions. Across these two synthetic corpora, TF--IDF with logistic regression therefore shows strong within-corpus performance but limited and asymmetric transfer.
