...
\item[RQ1] What internal performance is supported for HiED, and what comparisons can be made with the available baseline evidence?
...
The 1,000-case internal held-out set was sampled from the 14,000-case training source using the first-listed projected parent label to approximately match the public-validation class distribution. A fixed random seed was used for reproducibility. The intended experimental contract assigned the remaining 13,000 cases to conventional-baseline training and retrieval-index construction. The first-listed label served only as a stratification variable and was not treated as a clinically validated primary diagnosis. No additional semantic near-duplicate screening was performed.

The frozen repository retains the held-out case-ID fingerprint and the complete retained HiED output, but it does not retain the raw 13,000-case conventional-baseline training source together with a matching TF--IDF held-out prediction file. The exclusion and decoding contract for the historical TF--IDF headline row therefore cannot be independently rerun from the frozen snapshot. This provenance limit is handled explicitly in Section~\ref{sec:metric-audit-boundary}; the unsupported held-out TF--IDF row is not used as a result.
...
The retained LLM configurations were intended to be evaluated on the same fixed 1,000-case held-out set. The retained HiED row is reproducible from complete case-level output; the Single and Majority rows are available only as archived aggregate summaries, and the historical TF--IDF held-out row lacks a matching artifact. Because the public-validation split was used during system development, its results are not interpreted as an independent test estimate and are not substituted for missing held-out evidence.
...
\item The \emph{multilabel view}, $M(x)$, is the complete final set of emitted parent labels used jointly for Exact Match, Macro-F1, and Weighted-F1. HiED--DA contributes its committed primary diagnosis and any emitted comorbid parent label. Single contributes its parsed primary diagnosis and any explicitly emitted additional or comorbid diagnosis. A one-label method contributes a singleton set. A score-based multilabel classifier requires a frozen decoding rule to convert class scores into $M(x)$.
\end{enumerate}

\begin{table}[H]
\centering
\caption{Method-specific construction of the prediction objects used by the main metrics.}
\label{tab:method-output-contract}
\small
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{2.45cm}|>{\raggedright\arraybackslash}p{3.0cm}|>{\raggedright\arraybackslash}p{3.1cm}|X|}
\hline
Method & Primary view $p_1(x)$ & Ranked view $R_3(x)$ & Complete emitted set $M(x)$ and evidence status\\
\hline
Majority & Most frequent scoring parent & No genuine ranking & Singleton primary set; only an archived aggregate internal result is retained\\
\hline
TF--IDF + LR & Highest-scoring class & Three highest-scoring classes & Requires one frozen score decoder. The preserved S8 case file is public-validation evidence and does not match the internal held-out case universe; no matching held-out emitted-set artifact supports the historical headline row\\
\hline
Single & Parsed committed primary & No standardized genuine ranking & Parsed primary plus explicitly emitted additional or comorbid diagnoses; the internal values are retained aggregate records without a matching complete case-level file\\
\hline
HiED--DA & Committed primary & Recorded Diagnostician ranking & Committed primary plus optional emitted comorbid diagnosis; the retained internal row is reproduced from complete case-level output\\
\hline
\end{tabularx}
\end{table}
...
Because a case may contain more than one gold label, each hit measure counts a case as correct when at least one projected benchmark gold label appears in the evaluated view. Exact Match, also called subset accuracy, instead requires the complete predicted label set to equal the complete gold-label set. Exact Match, Macro-F1, and Weighted-F1 are always calculated from the same emitted set $M(x)$; Exact Match is not calculated from Top-1 while F1 uses a separate thresholded output. Their trends may nevertheless differ because Exact Match gives all-or-none case-level credit, whereas F1 gives label-level partial credit and is affected by false-positive and false-negative labels. These are benchmark-agreement measures, not independent measures of the patient's true clinical diagnosis.
...
% CH4_METRIC_CONTRACT_AUDIT_V2
\subsection{Case-Level Metric Audit and Recalculation Boundary}
\label{sec:metric-audit-boundary}

The metric implementation and preserved outputs were audited before interpreting the Chapter~\ref{ch:results} headline table. The current evaluator uses $p_1(x)$ for Top-1, a genuine ordered list for ranked Top-3, and the same emitted set $M(x)$ for Exact Match, Macro-F1, and Weighted-F1. The apparent divergence between Exact Match and F1 is therefore not caused by Exact Match using Top-1 while F1 uses a different threshold. It can arise because Exact Match is an all-or-none set equality measure, while F1 gives partial label-level credit and changes with output cardinality.

The frozen 1,000-case output of the validation-selected HiED--DA parent-balanced configuration was independently rescored. The recalculation produced Top-1 Accuracy 0.518000, ranked Top-3 Accuracy 0.802000, Exact Match 0.409000, Macro-F1 0.177577, and Weighted-F1 0.433738, reproducing the reported three-decimal values. Its mean gold-set size is 1.093 and its mean predicted-set size is 1.136. HiED--DA emits one diagnosis for 864 cases and two diagnoses for 136 cases.

The only preserved TF--IDF case-level file associated with the earlier baseline lineage is the 1,000-case S8 file. It contains class probabilities and rankings, but no explicitly emitted additional diagnosis. Its case identifiers have zero case-identifier overlap with the 1,000-case retained HiED held-out trace. It is therefore treated as public-validation evidence, regardless of older summary metadata, and cannot support a same-split held-out comparison.

\begin{table}[H]
\centering
\caption{Diagnostic rescoring of the preserved S8 TF--IDF public-validation output under explicit emitted-set decoders. These views are decoder sensitivities, not replacement held-out results.}
\label{tab:tfidf-decoder-audit}
\small
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{|l|c|c|c|c|c|c|}
\hline
Emitted-set decoder & Top-1 & Ranked Top-3 & Exact Match & Macro-F1 & Weighted-F1 & Mean $|M(x)|$\\
\hline
Stored emitted set / primary only & 0.610 & 0.919 & 0.553 & 0.354 & 0.581 & 1.000\\
\hline
Primary plus second label if score $\geq 0.30$ & 0.610 & 0.919 & 0.053 & 0.352 & 0.585 & 1.954\\
\hline
All labels with score $\geq 0.50$ & 0.610 & 0.919 & 0.327 & 0.404 & 0.603 & 1.614\\
\hline
All labels with score $\geq 0.30$ & 0.610 & 0.919 & 0.043 & 0.305 & 0.547 & 2.960\\
\hline
\end{tabular}
\end{adjustbox}
\end{table}

None of these decoders reproduces the historical internal headline tuple of Top-1 0.632, Top-3 0.923, Exact Match 0.321, Macro-F1 0.393, and Weighted-F1 0.601. Top-1 remains 0.610 under every emitted-set decoder, so a threshold choice cannot explain the 0.632 headline value. The problem is therefore missing or inconsistent run provenance, not merely a different threshold for F1.

Accordingly, the historical held-out TF--IDF headline row is withdrawn from the same-split comparison and is not replaced by the S8 validation result. A valid replacement requires the intended 13,000-case training source, verified exclusion of all held-out case IDs, a decoder selected using development or validation data only, and a retained case-level held-out prediction file. Majority and Single aggregate values are shown only with an explicit archived-aggregate status; they are not used for new paired, overlap, or output-cardinality claims.
...
% CH5_EXPERIMENTAL_DESIGN_REWRITE_V1
\chapter{Experimental Design}
\label{ch:experimental}

This chapter explains how each research question is tested under the data,
output, metric, and evidence rules defined in Chapter~\ref{ch:data}. It keeps
five parts separate: validation-based configuration selection, internal
held-out evaluation, case-level stage-wise analysis, synthetic transfer
analysis, and the planned psychiatrist annotation pilot. These parts use
different case groups and support different conclusions.

\section{Study Sequence and Evidence Classes}
\label{sec:experimental-overview}

The study begins with retrieval-configuration selection on the LingxiDiag
public-validation split. The selected Single and HiED configurations are then
frozen before the internal held-out results are interpreted. The retained HiED
case-level output is used for the main internal estimate, disagreement
localization, and matched primary-selection comparisons. MDD-5k and a separate
cross-corpus lexical experiment are used to examine sensitivity to a second
synthetic source. A blinded psychiatrist annotation pilot is planned for a
small selected set of LingxiDiag disagreement cases.

The saved evidence is not equally complete for every internal method. HiED--DA
has a complete case-level held-out output. The available Single and Majority
values are archived aggregate records. The historical TF--IDF held-out row
does not have a matching case-level file and is not used as an internal result.
A comparison is treated as paired only when both methods cover the same case
identifiers under the same scoring rule.

\begin{table}[htbp]
\centering
\caption{Study populations, evidence classes, and their roles.}
...
\hline
\textbf{Study part} & \textbf{Population or evidence} & \textbf{Role and boundary}\\
\hline
Retrieval selection (RQ3) & LingxiDiag public validation, $N=1000$ & Selects one retrieval configuration separately for Single and HiED; used as development evidence, not an independent test estimate\\
\hline
Internal HiED evaluation (RQ1--RQ4) & Fixed LingxiDiag held-out set, $N=1000$; complete retained HiED trace & Supports the retained HiED estimate, stage-wise localization, and matched DA--NtS comparisons\\
\hline
Internal baseline context (RQ1) & Majority and Single aggregate records; historical TF--IDF row withdrawn & Provides descriptive context only; unsupported paired, overlap, threshold, and output-size claims are not made\\
\hline
Synthetic transfer and sensitivity (RQ5) & Internal held-out set; MDD-5k $N=925$ with 878 checker-eligible cases & Examines model-scale and source sensitivity; does not establish real-patient validity\\
\hline
Planned psychiatrist pilot & Twenty selected LingxiDiag cases from the 272-case $D_3=1$, $I=1$, $S=0$ group & Defines a pending feasibility and exploratory agreement study on synthetic transcripts\\
\hline
\end{tabularx}
\end{table}

All completed computational analyses use synthetic Chinese psychiatric
dialogues. The planned psychiatrist pilot also uses selected synthetic
LingxiDiag transcripts. No real-patient transcript or prospective clinical
evaluation is included in the completed study.

\section{Retrieval-Configuration Selection and Internal Benchmark Evidence}
\label{sec:benchmark-retrieval-design}

\subsection{Compared Retrieval Configurations}

Retrieval is evaluated separately within the Single and HiED--DA architectures
under three complete configurations: no retrieval, global Top-5 retrieval, and
parent-balanced retrieval. Within each architecture, the backbone model,
prompt family, diagnostic scope, decoding settings, and output contract remain
fixed.

Global Top-5 and parent-balanced retrieval may differ in demonstration count,
label composition, ordering, similarity distribution, and total prompt length.
Their comparison therefore evaluates complete retrieval configurations rather
than the isolated causal effect of label balancing. Retrieved examples are
supplied only to the diagnosis path. The Criterion Checkers continue to
evaluate the current transcript without retrieved demonstrations, as defined
in Chapter~\ref{ch:architecture}.

\subsection{Validation-Only Configuration Selection}

Retrieval configurations are selected using the LingxiDiag public-validation
split only. The selected configuration for each architecture is frozen before
the internal held-out results are interpreted. Held-out results are therefore
post-selection evaluations and are not used to revise the retained
configuration.

Single and HiED have different three-label output contracts. Single does not
produce a standardized ranked differential diagnosis, so its three-label
quantity is an emitted-label hit@3. HiED produces a genuine Diagnostician
ranking and is evaluated using ranked Top-3 Accuracy. These quantities are used
only within their own architecture and are not treated as one common
cross-architecture measure.

For Single, the retained configuration follows its archived
architecture-specific validation-selection record. For HiED--DA,
parent-balanced retrieval is the initial retained configuration. Global Top-5
may replace it only when the validation record passes the required completeness
and regression checks and is strictly higher on all five prespecified headline
measures: committed Top-1 Accuracy, genuine ranked Top-3 Accuracy, Exact Match,
Macro-F1, and Weighted-F1. This is a conservative selection rule rather than a
statistical hypothesis test. The validation outcomes and retained
configurations are reported in Chapter~\ref{ch:results}.

\subsection{Internal Benchmark Evidence and Provenance Boundary}

The intended RQ1 design compares the validation-selected HiED--DA and Single
configurations with a Majority baseline and a conventional TF--IDF classifier
on one fixed 1,000-case internal held-out set under the parent-label rules in
Chapter~\ref{ch:data}. Committed Top-1 Accuracy is the common primary-output
measure. Exact Match, Macro-F1, and Weighted-F1 use each method's complete
emitted set. Ranked Top-3 is reported only when a genuine ordered output is
available.

The saved evidence does not fully realize this intended comparison. A complete
case-level file reproduces the retained HiED--DA row. Single and Majority are
available only as archived aggregate records. The only preserved TF--IDF
case-level output belongs to the public-validation population and has no
case-identifier overlap with the retained internal held-out trace. The
historical TF--IDF held-out row is therefore withdrawn rather than treated as a
same-split result.

Chapter~\ref{ch:results} reports the evidence status beside each internal row.
Aggregate-only records are interpreted descriptively and do not support new
paired confidence intervals, McNemar tests, prediction-overlap analyses, or
output-cardinality comparisons. The public-validation TF--IDF result is not
substituted for the missing held-out baseline.

Accordingly, RQ1 can establish the retained HiED internal estimate and describe
its relationship to the available baseline records. A reproducible
conventional-classifier comparison cannot be established from the preserved
repository snapshot.

\section{Recorded-Output Disagreement Localization}
\label{sec:stage-wise-analysis-design}

RQ2 examines where disagreement with the projected gold labels is recorded in
the retained HiED--DA trace. It uses three case-level indicators defined in
Section~\ref{sec:case-level-indicators}: candidate detection in the genuine
Top-3, $D_3$; gold-label inclusion in the criterion-compatible set, $I$; and
committed-primary agreement, $S$.

$D_3$ and $S$ are available for all 1,000 internal cases. Analyses involving
$I$ are limited to the 915 checker-eligible cases whose gold-label set contains
at least one non-\emph{Others} parent represented by a Criterion Checker. The
three indicators describe different saved output views. They are not treated
as a causal or chronological funnel. For example, a diagnosis may be absent
from the Top-3 but present in the compatible set because all configured
diagnoses are checked independently.

Checker-eligible cases are assigned to four mutually exclusive groups shown in
Table~\ref{tab:disagreement-groups}. Assignment first separates cases by
committed-primary agreement. Cases with disagreement are then separated by
candidate detection and gold-label inclusion. This is an analysis rule, not a
claim about the runtime order of every policy.
...
\textit{Note.} Under the DA ranking contract, $S=1$ implies $D_3=1$. A dash means that gold-label inclusion does not affect assignment to that group.
}
\end{table}

The fourth group is the strongest recorded profile consistent with a
primary-selection mismatch: a gold label appears in both the genuine Top-3 and
the criterion-compatible set but is not committed as primary. This profile does
not prove that the finalizer alone caused the disagreement. The ranking,
criterion states, compatible set, and committed diagnosis are connected
model-derived outputs, and compatibility is not a ranked recommendation of the
primary diagnosis.

\paragraph{Supplementary gold-informed oracle.}
\label{sec:oracle-design}
A gold-informed oracle estimates optimistic headroom within the saved ranked and
compatible outputs. It selects a gold label when the prespecified oracle rules
allow it and leaves already matching cases unchanged. Because it directly uses
gold labels and assumes that eligible changes create no new errors, it is
non-deployable and is reported only as an assumption-dependent upper bound. The
candidate pool and sensitivity rules are given in Appendix~\ref{app:supporting}.

\section{Primary-Selection Experiments}
\label{sec:primary-selection-intervention-design}

RQ4 asks whether another selection method converts the available diagnostic
records into better committed-primary agreement than Direct-Answer (DA). The
design separates matched policy comparisons that reuse one fixed trace from
complete reruns that introduce new model inference or repeated sampling.

\subsection{Matched DA--NtS Comparison}

The main controlled comparison evaluates DA against Nominate-then-Select (NtS).
Within each retrieval condition, both policies reuse the same Diagnostician
ranking, criterion states, and criterion-compatible set. DA keeps the
Diagnostician's proposed primary diagnosis. NtS applies the deterministic
selection rule defined in Chapter~\ref{ch:architecture}. NtS introduces no new
LLM inference.

DA and NtS are compared on the same 1,000 internal held-out cases under no
retrieval, global Top-5 retrieval, and parent-balanced retrieval. The primary
measure is the paired difference in committed Top-1 correctness. These three
comparisons form one prespecified family and receive Holm adjustment.
Paired-bootstrap confidence intervals and exact McNemar tests follow
Chapter~\ref{ch:data}. Retrieval-by-finalization difference-in-differences
contrasts are secondary analyses.

Because DA and NtS share the same upstream records, committed Top-1 provides the
clearest test of the two selection policies. Exact Match, Macro-F1, and
Weighted-F1 remain descriptive for this contrast because DA may emit one
additional comorbid diagnosis while NtS emits one diagnosis. Differences in
these set-based measures cannot be assigned to primary re-selection alone.

\subsection{Supporting Methods and Provenance Classes}

Table~\ref{tab:selection-method-provenance} groups the additional methods by
what changes during evaluation.

\begin{table}[htbp]
\centering
\caption{Evidence classes for supporting primary-selection analyses.}
\label{tab:selection-method-provenance}
\small
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3.4cm}|>{\raggedright\arraybackslash}p{4.1cm}|X|}
\hline
\textbf{Evidence class} & \textbf{Methods} & \textbf{Interpretation boundary}\\
\hline
Fixed recorded outputs & NtS; deterministic fusion; external forced-pairwise replay & Changes a deterministic decision rule on one saved upstream trace; isolates policy choice only when the matched trace is retained\\
\hline
New-inference configurations & Independent pairwise comparison; two-view debate & Runs model-based stages again and may change upstream outputs; evaluated as a complete configuration rather than a finalizer-only test\\
\hline
Repeated-generation configurations & Self-consistency; confidence-informed self-consistency & Combines repeated Diagnostician generations under a larger sampling budget; does not reproduce the fixed DA trace\\
\hline
\end{tabularx}
\end{table}

Deterministic fusion applies one validation-selected rule to saved ranking,
compatibility, and criterion-support summaries. Independent pairwise comparison
and two-view debate add model calls. The retained internal pairwise result comes
from a separate full-pipeline run, so any difference from DA may also include
changes in upstream outputs. Self-consistency methods combine repeated
Diagnostician generations and are treated as complete sampling-based
configurations.

Forced pairwise override is reported only in the external MDD-5k analysis,
where it is a deterministic replay of retained pairwise preferences on a
matched trace. No internal forced-override result is reported because the saved
internal outputs do not establish a matched forced-commit configuration.

Committed Top-1 Accuracy is the common RQ4 outcome. A paired interval or
McNemar test is reported only when both methods retain complete outputs for the
same case identifiers. Aggregate-only comparisons remain descriptive. Detailed
rules, inference budgets, tie handling, and paired statistics are reported in
Appendix~\ref{app:supporting}.

\section{Sensitivity and Synthetic Transfer Analyses}
\label{sec:sensitivity-transfer-design}

RQ5 examines whether the recorded stage-wise views remain informative under
model-scale variation and a second synthetic dialogue source. Supporting
analyses describe lexical source sensitivity and common disagreement
directions. These analyses do not establish real-patient validity.

\paragraph{Within-family model-scale sensitivity.}
Qwen3-4B, 8B, 14B, and 32B are evaluated on the same fixed 1,000-case internal
held-out set while holding the prompt family, diagnostic scope, retrieval
policy, decoding contract, and metrics constant. Committed Top-1 Accuracy,
genuine ranked Top-3 Accuracy, structured-output failures, and abstention events
are recorded. This analysis is limited to the evaluated Qwen3 family and
serving setup.

\paragraph{External synthetic evaluation.}
MDD-5k provides a second Chinese synthetic dialogue source created under a
different process. The main external analysis uses all 925 patient-level cases
and separately reports the 878 cases for which at least one projected gold
diagnosis is represented in HiED's configured output space. These 878 cases are
checker-eligible and support the external $D_3$--$I$--$S$ analysis. A separate
490-case F32--F41 subset is treated as a secondary scope-restricted analysis,
not the main external estimate.

The external trace retains the committed primary diagnosis, Diagnostician
ranking, criterion states, and criterion-compatible set for each case. It
therefore supports candidate detection, gold-label inclusion,
committed-primary agreement, and matched policy replays. A validated matched
bundle for external retrieval-policy comparison is unavailable, so
retrieval-policy transfer is not claimed.

\paragraph{Cross-corpus lexical transfer.}
A separately preserved TF--IDF experiment trains the same classifier family on
each synthetic corpus and evaluates it within source and across sources. This
analysis tests whether lexical associations depend on the synthetic generation
source. It is a distinct full-corpus transfer experiment and is not used to
replace the missing internal held-out TF--IDF baseline.

\paragraph{Descriptive disagreement patterns.}
A first-listed-label confusion matrix, frequent reference-to-prediction pairs,
and the $D_3$--$I$--$S$ profiles are used to describe recurring disagreement
directions. The first-listed label is used only for a one-label visualization
and is not substituted for the main any-gold or multilabel evaluation.

The model-scale and MDD-5k analyses provide the main evidence for RQ5. The
lexical-transfer and confusion-pattern analyses provide supporting evidence
about source sensitivity and the limits of the evaluated synthetic setting.

\section{Planned Blinded Psychiatrist Annotation Pilot}
\label{sec:clinical-evaluation-plan}
\label{sec:lingxidiag-pilot-review}

The blinded psychiatrist annotation pilot is the only clinician-facing study
retained in the thesis protocol. At the time of writing, the annotation remains
pending. This section therefore defines the frozen review plan and planned
analyses; it does not contribute a completed clinical result to the
computational evaluation chapters.

The pilot uses twenty selected synthetic LingxiDiag transcripts. It does not
use real-patient records and does not constitute prospective clinical
validation. Its purpose is to examine whether the review procedure is workable
and to describe one selected synthetic disagreement population.

Twenty cases are sampled without replacement from the 272-case group satisfying
$D_3=1$, $I=1$, and $S=0$. The random seed and case identifiers are fixed before
review. Two psychiatrists independently review all cases in separately
randomized orders. They remain blinded to the benchmark labels, HiED outputs,
computational disagreement profile, case-selection reason, and the other
reviewer's responses.

Reviewers first read the transcript and record whether the available
information supports a unique primary diagnosis, a provisional primary
diagnosis, possible comorbid diagnoses, and up to three ranked differential
diagnoses. They then label the prespecified evidence items and
diagnosis-specific qualifiers as \texttt{met}, \texttt{not\_met}, or
\texttt{insufficient\_evidence}. After the evidence annotation, they may revise
their diagnostic judgments. Both provisional and final responses are retained.

Shared symptom and impairment concepts are annotated once. Diagnosis-specific
duration, frequency, timing, relationship, threshold, and exclusion conditions
remain separate. The mapping between the physician-facing evidence items and
the original Criterion Checker items is frozen before annotation.

After both reviewers complete their independent final assessments, the research
team will compare their responses with the LingxiDiag benchmark labels, HiED
ranked and committed diagnoses, criterion states, and system-derived
compatibility decisions. Pre-consensus inter-rater agreement will be calculated
from the independent responses. Any later consensus step will use the
transcript and independent annotations while benchmark labels and HiED outputs
remain hidden from the reviewers.

The planned analyses include transcript-only primary-diagnosis agreement,
ranked-differential coverage, criterion-state agreement, pre-consensus
inter-rater agreement, provisional-to-final diagnostic changes, and agreement
between psychiatrist-evidence-derived and system-derived compatibility
judgments.

Because the pilot contains only twenty synthetic cases selected from one
recorded disagreement group, any completed result will be interpreted as
feasibility and exploratory agreement evidence. It will not be treated as an
estimate of clinical diagnostic accuracy, population prevalence, workflow
benefit, or real-patient validity.

\section{Chapter Summary}

This chapter defined how each research question is evaluated under the data,
metric, output-view, and evidence contracts in Chapter~\ref{ch:data}. It
separated validation-based configuration selection from held-out evaluation,
matched case-level analyses from aggregate-only descriptive records, and
completed synthetic analyses from the planned psychiatrist annotation pilot.

The retained HiED trace supports internal stage-wise and primary-selection
analyses. The historical TF--IDF held-out row is not used because a matching
case-level artifact is unavailable. MDD-5k and the separate lexical-transfer
experiment provide bounded synthetic source-sensitivity analyses.
Chapter~\ref{ch:results} reports the internal results supported by these
contracts.

\chapter{Internal Evaluation Results}
\label{ch:results}

This chapter reports the internal results in the order needed to interpret the study. Retrieval configurations are selected first on the public-validation split (RQ3). The retained internal records are then interpreted under the evidence boundary defined in Chapters~\ref{ch:data} and~\ref{ch:experimental} (RQ1). The remaining sections localize disagreement across the recorded output views (RQ2), evaluate primary-selection interventions (RQ4), and report the internal model-scale and diagnostic-confusion analyses that contribute to RQ5. All results in this chapter measure agreement with projected benchmark labels on synthetic LingxiDiag dialogues; they are not estimates of clinical diagnostic accuracy.
...
This section addresses RQ1 under the evidence boundary established by the case-level audit. Single uses the validation-selected global Top-5 configuration, and HiED--DA uses the validation-selected parent-balanced configuration. The intended comparison concerns complete systems, but the preserved evidence is not equally complete across rows.

\begin{table}[H]
\centering
\caption{Internal benchmark records after the metric-contract audit. HiED--DA is reproduced from complete case-level output; Majority and Single are retained archived aggregates; the historical TF--IDF held-out row is withdrawn because no matching artifact exists.}
\label{tab:matched-baselines-retrieval}
\small
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{|l|l|>{\raggedright\arraybackslash}p{4.1cm}|c|c|c|c|c|c|}
\hline
System & Retrieval & Evidence status & Top-1 & \shortstack{Three-label\\coverage} & Exact Match & Macro-F1 & Weighted-F1 & Mean $|M(x)|$\\
\hline
Majority & None & Archived aggregate; one-label output contract & 0.398 & 0.398$^{\mathrm{E}}$ & 0.327 & 0.047 & 0.207 & 1.000\\
\hline
TF--IDF + LR & None & Held-out row withdrawn; no matching case-level artifact & --- & --- & --- & --- & --- & ---\\
\hline
Single & Global Top-5 & Archived aggregate; no matching complete case-level file & 0.517 & 0.748$^{\mathrm{E}}$ & 0.029 & 0.224 & 0.447 & ---\\
\hline
HiED--DA & Parent-balanced & Case-level verified retained configuration & 0.518 & 0.802$^{\mathrm{R}}$ & 0.409 & 0.178 & 0.434 & 1.136\\
\hline
\end{tabular}
\end{adjustbox}
\parbox{0.96\textwidth}{\footnotesize
\textit{Note.} $^{\mathrm{R}}$ denotes genuine ranked Top-3 Accuracy and $^{\mathrm{E}}$ denotes emitted-label hit@3. These are different output views. A dash means that the value is not supported under the required held-out artifact contract; it is not a zero. Exact Match and F1 use the same emitted set within a row, but cross-method set metrics also depend on how each system constructs that set.
}
\end{table}

The retained HiED--DA case-level output gives Top-1 Accuracy 0.518 and genuine ranked Top-3 Accuracy 0.802. The archived Single aggregate gives Top-1 0.517. Their 0.001 difference is descriptive only because a matching complete Single case-level file was not retained; no paired confidence interval, McNemar test, case-overlap analysis, or output-cardinality comparison is inferred.

The Exact Match difference between the HiED aggregate row and the Single aggregate row does not demonstrate a multi-agent advantage. HiED--DA has a verified mean emitted-set size of 1.136, whereas the corresponding Single cardinality record is unavailable. Exact Match is all-or-none, and Macro-F1 and Weighted-F1 give partial label-level credit. These measures can move differently even though all three read the same emitted set within each method.

No numerical internal conclusion is drawn for TF--IDF with logistic regression. The preserved S8 TF--IDF file has zero case overlap with the retained held-out trace and yields Top-1 0.610 rather than the historical 0.632 under every audited emitted-set decoder. Its validation metrics and decoder sensitivities are reported in Section~\ref{sec:metric-audit-boundary}, but they are not substituted into this held-out table.

HiED therefore does not establish an overall final-label accuracy advantage from the preserved internal evidence. Its distinct empirical contribution is the retained structured trace: a genuine ranked differential, criterion states, a criterion-compatible set, missing-information records, and a committed-primary record. Those artifacts support the stage-wise analyses that follow.
...
The retained HiED--DA row has case-level verified Top-1 Accuracy of 0.518, while the archived Single aggregate is 0.517. This near-equality is descriptive because the matching complete Single case file is unavailable. The intended same-split conventional TF--IDF comparison cannot be established from the preserved repository snapshot, so no claim is made that it outperforms or underperforms HiED on the internal held-out set. HiED does not establish an accuracy advantage, but it provides the diagnostic records needed for the stage-wise analyses below.
...
The internal results give five main answers. Retrieval improves the archived Single aggregate more clearly than HiED, and parent-balanced retrieval remains the primary HiED configuration because of the validation contract rather than the held-out ranking. The retained HiED Top-1 result is nearly identical to the archived Single aggregate, while the intended conventional TF--IDF held-out comparison remains unresolved because its matching artifact was not retained. HiED nevertheless exposes a large difference between candidate availability, criterion compatibility, and committed-primary agreement: 272 of 915 checker-eligible cases contain a benchmark gold parent in both the Top-3 and compatible set but commit another diagnosis. The tested primary-selection interventions do not provide a clear attributable improvement over DA. Finally, HiED's Top-3 coverage is comparatively stable across the evaluated Qwen3 sizes, while the remaining disagreements are concentrated toward a small number of diagnostic directions. Chapter~\ref{ch:external} examines whether these patterns persist under external synthetic distribution shift.
...
The results support a stage-wise interpretation of HiED rather than an accuracy-leading claim. The retained HiED case-level Top-1 result and the archived Single aggregate are nearly identical, while the historical TF--IDF held-out comparison cannot be established from the preserved evidence. HiED's distinct contribution is the set of recorded outputs that makes candidate generation, criterion checking, compatibility analysis, and primary commitment separately visible.
...
The separately preserved cross-corpus TF--IDF matrix shows that within-source prediction can depend strongly on wording and label priors. It is a distinct full-corpus transfer artifact and does not repair the missing internal held-out baseline. The completed evidence therefore supports a bounded second-synthetic-source test, not a recovered same-split TF--IDF headline comparison. It does not establish generalization to real psychiatric consultations, independent hospitals, spontaneous or code-switched Taiwanese conversations, or Taiwanese coding practice.

\section{Model and Experimental-Comparison Limitations}
\label{sec:model-comparison-limitations}

Most comparisons change more than one component, and the internal baseline evidence is not equally preserved. The retained HiED row is case-level verified, whereas Single and Majority are archived aggregates and the historical TF--IDF held-out row has been withdrawn. Even a future fully matched comparison would still differ in training, retrieval, prompts, model calls, intermediate outputs, and output contract, so performance differences could not be assigned only to multi-agent orchestration.
...
HiED does not establish the best final-label accuracy. Its retained internal Top-1 result is almost identical to the archived Single aggregate. The intended same-split TF--IDF comparison cannot be resolved from the preserved case-level evidence and is therefore not used to rank the methods. HiED's main technical value is that its recorded outputs allow benchmark disagreement to be divided into different profiles.
...
\item[RQ1] \textbf{What internal performance is supported for HiED, and what comparisons can be made with the available baseline evidence?}

The retained HiED case-level Top-1 result and the archived Single aggregate are almost identical. A reproducible same-split TF--IDF comparison cannot be established from the frozen repository, so the historical baseline row is not used to rank the systems. HiED does not establish an overall accuracy advantage, but it provides a genuine ranked differential, criterion states, a criterion-compatible set, and a final selection record for stage-wise analysis.
...
{\footnotesize
\noindent\textit{TF--IDF baseline evidence boundary:} The intended conventional baseline used character-boundary 1--2-grams, at most 10,000 features, \texttt{min\_df=2}, \texttt{max\_df=0.95}, sublinear term frequency, and twelve class-balanced one-vs-rest L-BFGS logistic-regression classifiers with $C=1.0$ and a 2,000-iteration limit. The frozen snapshot does not retain the intended 13,000-case training source together with a matching held-out prediction file and frozen emitted-set decoder. The preserved S8 case output belongs to a non-overlapping public-validation population. Its probabilities support a genuine ranking, but thresholded emitted sets in this thesis are diagnostic sensitivities only and are not presented as a recovered held-out result.
\par}
