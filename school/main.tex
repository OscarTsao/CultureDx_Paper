...
The most capable diagnostic-reasoning systems set an informative reference point. AMIE, a large language model optimized for diagnostic reasoning, generated differential lists that improved clinicians' diagnostic accuracy on complex real-world cases~\citep{mcduff2025amie}. AMIE differs from the present work in setting and goal: it targets open-ended differential quality on English internal-medicine cases and is evaluated by expert raters, whereas this thesis holds a Chinese psychiatric transcript fixed and asks where a decomposed pipeline diverges across detection, verification, and committed selection. The two are also complementary on interpretability. AMIE's own evaluation notes that for complex cases it can draw conclusions from isolated symptoms rather than viewing the case holistically, and its reasoning is not auditable at the level of individual criteria---the gap this thesis addresses with an explicit ICD-10 criterion trace.
