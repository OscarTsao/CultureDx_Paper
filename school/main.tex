...
First, this thesis defines a same-case stage-wise output and evaluation contract, implemented through a two-path architecture for psychiatric decision support. Compared with a Single LLM that mainly returns a final diagnostic output, HiED separately records the ranked candidate list, criterion-checking results, criterion-compatible set, and committed primary diagnosis. This design allows candidate generation, criterion checking, and primary diagnosis selection to be studied separately. Using these outputs, this thesis finds that primary commitment contains the largest recorded benchmark-disagreement profile in the two tested synthetic datasets. This is a benchmark finding under the evaluated settings, not a claim about the true clinical primary diagnosis in every case.
...
This chapter describes the completed configuration-selection, held-out-evaluation, primary-selection, and supporting boundary analyses. These analyses use different populations and support different claims. Validation results select configurations; the internal held-out set provides the main internal estimate; and MDD-5k provides the second-synthetic-source stage-wise analysis. A separate final section documents a proposed post-thesis psychiatrist review protocol for selected synthetic cases. That protocol was not conducted within the thesis period and is not part of the completed evidence.
...
\hline
Gold label ranked and criterion-compatible but not committed as primary & 0 & 1 & 1 \\
\hline
\end{tabularx}
...
\section{External Portability and Supporting Boundary Analyses}
...
\paragraph{Diagnostic-scope boundary.}
...
\section{Proposed Single-Psychiatrist Review of Benchmark-Label Alignment (Future Work)}
...
The strong same-corpus lexical result is consistent with substantial predictive information in transcript wording, label frequency, and other corpus-specific regularities. The cross-corpus analysis in Chapter~\ref{ch:external} tests whether this advantage persists when the source distribution changes. HiED's distinct empirical contribution in the internal analysis is not higher final-label accuracy, but the additional recorded artifacts: a genuine ranked differential, criterion states, a criterion-compatible set, and a committed-primary record.
...
Under the common internal split and parent-label mapping, TF--IDF with logistic regression provides the strongest same-corpus primary-label and class-level F1 performance. Single and HiED--DA have almost identical committed Top-1 performance. HiED does not establish an internal accuracy advantage, but it provides the diagnostic records needed for the stage-wise analyses below. Whether the lexical advantage persists across corpus sources is examined in Chapter~\ref{ch:external}.
...
\hline
Yes & Yes & No & 272 (29.7\%) & Ranked and compatible; not committed as primary\\
\hline
...
The six profiles in Table~\ref{tab:d3rs-joint} form the four analytical groups defined in Chapter~\ref{ch:experimental}: 518 cases with committed-primary agreement, 113 Top-3 candidate misses, 12 cases in which a gold parent is ranked but absent from the compatible set and another diagnosis is committed as primary, and 272 cases in which a gold parent is ranked and compatible but another diagnosis is committed as primary. The 79 cases with $D_3=0$ and $I=1$ are possible because all configured categories are checked, not only the Diagnostician's Top-3.
...
\section{Supporting Model-Scale and Diagnostic Disagreement Analyses}
...
\paragraph{Supporting internal boundary findings.}
...
The internal results give five main answers. Retrieval is associated with larger Top-1 point-estimate differences for Single than for HiED, and parent-balanced retrieval remains the primary HiED configuration because of the validation contract rather than the held-out ranking. TF--IDF with logistic regression provides the strongest same-corpus primary-label and class-level F1 performance, while Single and HiED have almost identical committed Top-1 performance. HiED nevertheless exposes a large difference between candidate availability, criterion compatibility, and committed-primary agreement: 272 of 915 checker-eligible cases contain a benchmark gold parent in both the Top-3 and compatible set but commit another diagnosis. The tested primary-selection interventions do not provide a clear attributable improvement over DA. Finally, HiED's Top-3 coverage is comparatively stable across the evaluated Qwen3 sizes, while the remaining disagreements are concentrated toward a small number of diagnostic directions. Chapter~\ref{ch:external} examines whether these patterns persist under external synthetic distribution shift.
...
\end{adjustbox}
\parbox{0.96\textwidth}{\footnotesize
\textit{Note.} $^{*}$The internal compatibility value uses the 915 checker-eligible LingxiDiag cases rather than all 1,000 cases. The MDD-5k all-case row uses all 925 cases, whereas the checker-eligible row excludes 47 cases whose benchmark diagnoses are outside the configured checker scope. The checker-eligible row is therefore a conditional analysis and should not be interpreted as a direct improvement over the all-case result.}
\end{table}
...
\section{Cross-Corpus Transfer of the TF--IDF + LR Baseline}
\label{sec:external-lexical-transfer}

Chapter~\ref{ch:results} showed that TF--IDF with logistic regression is a strong same-corpus lexical classifier on LingxiDiag. This section tests whether that advantage persists when the training and evaluation sources differ. The analysis uses a separately preserved full-corpus TF--IDF with logistic-regression matrix. Table~\ref{tab:lexical-transfer-rewrite} reports Top-1 and genuine Top-3 values under that artifact's twelve-class parent-label contract. Because this matrix comes from a separate transfer artifact, its within-corpus cells are used to interpret source sensitivity and are not substituted for the main internal headline row.

\begin{table}[htbp]
\centering
\caption{Cross-corpus transfer of TF--IDF with logistic regression under the preserved training and evaluation splits.}
...
\end{tabularx}
\parbox{0.96\textwidth}{\footnotesize
\textit{Note.} The LingxiDiag-trained classifier is evaluated on the 1,000-case LingxiDiag held-out set and the full 925-case MDD-5k set. For the MDD-trained conditions, MDD-5k is divided into an 80/20 training and held-out split using seed 42; the source-matched MDD result therefore uses 185 held-out cases. The table compares source-matched and cross-source behavior rather than four identically constructed performance estimates. These runs are not the previously reported F32--F41 subset experiment.}
\end{table}

TF--IDF with logistic regression performs substantially better when the training and evaluation data come from the same corpus. The LingxiDiag-trained classifier reaches a Top-1 Accuracy of 0.602 on the LingxiDiag held-out set but only 0.059 on the full MDD-5k set. The MDD-trained classifier reaches 0.632 on the 185-case MDD held-out split and 0.476 on the LingxiDiag held-out set. The degradation appears in both transfer directions but is asymmetric, suggesting substantial dependence on source-specific wording, dialogue-generation patterns, and label distributions.

These results do not support the absolute claim that TF--IDF with logistic regression cannot generalize, and they do not prove that HiED generalizes better. They show that a strong same-corpus lexical score should not be treated as evidence of source-independent diagnosis performance. They also do not establish clinical generalization for either architecture.
...
Finally, the lexical transfer matrix shows large and asymmetric source effects: the strong within-corpus TF--IDF performance does not persist under either transfer direction. Together, these results establish a synthetic cross-corpus boundary for the study. They do not establish superior generalization by HiED, real-patient validity, a uniquely correct clinical primary diagnosis, or improved clinical efficiency.
...
\ThesisFigure[0.72\textwidth]{fig_ch8_reference_rank.pdf}{Rank of the highest-ranked benchmark-reference parent among the 272 cases with $D_3=1$, $I=1$, and $S=0$, in which no benchmark-reference parent is committed as primary.}{fig:ch8-reference-rank}
\FloatBarrier
...
\begin{table}[htbp]
\centering
\caption{Most frequent benchmark-reference-to-committed-primary directions in the 272-case primary-commitment disagreement group.}
...
HiED's completed contribution is a stage-wise and inspectable output contract rather than a clinically validated diagnostic tool. The proposed single-psychiatrist review may provide a preliminary audit of benchmark-label alignment in selected synthetic disagreement cases, while broader clinical validation and deployment remain independent future research.
